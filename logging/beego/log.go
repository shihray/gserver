...
func (bl *BeeLogger) formatText(when time.Time, span *BeegoTraceSpan, logLevel int, msg string, v ...interface{}) (string, error) {
	...
	if bl.enableFuncCallDepth {
		...
		splitCall := strings.Split(CallStack, "\n")
		frontCall := strings.Split(splitCall[2], "/")
		frontCallString := strings.Join(frontCall[len(frontCall)-2:], "/")
		if logLevel <= LevelWarn {
			msg = fmt.Sprintf("[%s/%s] %s %s", frontCallString, ShortFile, msg, CallStack)
		} else {
			msg = fmt.Sprintf("[%s/%s] %s", frontCallString, ShortFile, msg)
		}
	}
