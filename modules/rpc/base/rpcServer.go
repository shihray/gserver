			...
			if !ok {
				...
				if callInfo.RpcInfo.Expired < (time.Now().UnixNano() / 1000000) {
					...
					if s.listener != nil {
						...
						logging.Warning("timeout: This is Call", s.module.GetType(), callInfo.RpcInfo.Fn, callInfo.RpcInfo.Expired, time.Now().UnixNano()/1000000)
					}
				} else {
					s.runFunc(callInfo)
				}
...
func (s *RPCServer) doCallback(callInfo mqrpc.CallInfo) {
	if callInfo.RpcInfo.Reply {
		...
		if err != nil {
			logging.Warning("rpc callback erro :\n%s", err.Error())
		}
	} else {
...
func (s *RPCServer) runFunc(callInfo mqrpc.CallInfo) {
	...
	_runFunc := func() {
		...
		// f 為函數地址
		var session melody.Session = nil
		...
		callInfo.ExecTime = time.Since(start).Nanoseconds()
		s.doCallback(callInfo)
		logging.Debug(span, "RPC Exec ModuleType = %v Func = %v Elapsed = %v", s.module.GetType(), callInfo.RpcInfo.Fn, time.Since(start))
		if s.listener != nil {
